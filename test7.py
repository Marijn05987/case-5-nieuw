...
# Zet de 'Unnamed: 0' kolom om naar een datetime-object voor weerdata
weer_data = pd.read_csv('weather_london.csv')
...
if not filtered_data_week.empty:
    ...
    # Deel 2: Checkboxen om grafieken te kiezen
    ...
    # Deel 3: Gecombineerde grafieken genereren
    ...
    if show_rentals:
        sns.lineplot(data=filtered_data_week, x='Date', y='Aantal Verhuurde Fietsen', ax=ax, label='Aantal Verhuurde Fietsen', color='purple')
...
# X en Y variabelen
x = filtered_data_week[weerfactor]  # Weerfactor (bijv. temperatuur)
y = filtered_data_week["Aantal Verhuurde Fietsen"]  # Aantal fietsverhuringen
